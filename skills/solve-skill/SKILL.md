...
- 复杂设计或实现任务必须在任务目录或任务文档旁生成：`context.md`、`plan.draft.md`、`research.md`、`decision.md`、`risks.md`、`plan.md`、`spec.md`、`tasks.md`、`verify.md`
...
- 没有最小合同，不允许深入查证，也不允许进入系统性 Locate / Plan Draft / Research / Plan / Implement。
...
没有 Evidence Map 不允许进入 Plan Draft / Research / Plan / Implement。
...
Confirm → Locate → Plan Draft → Research → Decision → Plan → Spec → Tasks → Implement → Review → Validate → Deliver

`plan.draft.md` 只是初步设计草案，不能直接用于实现。真正允许进入实现的依据必须是经过技术事实验证并回写修正后的 `plan.md`，以及由它拆出的 `spec.md`、`tasks.md` 和 `verify.md`。
...
复杂设计或实现任务必须把阶段产物写入文件，不允许只在对话中描述。

- `context.md`：项目背景、现有代码、依赖版本、运行环境、权限与沙箱约束
- `plan.draft.md`：初步设计、关键假设、计划改动、复用策略、待验证点
- `research.md`：技术事实搜集、依据来源、假设验证结果、设计修正
- `decision.md`：采用方案、放弃方案、取舍理由、依据来源、实现影响
- `risks.md`：剩余风险、触发条件、规避方案、降级方案、验收项
- `plan.md`：经过 Research / Decision 修正后的正式计划
- `spec.md`：可执行规格、接口/模块边界、数据结构、错误处理、验收标准
- `tasks.md`：任务清单、依赖顺序、涉及文件、风险任务、验收方式
- `verify.md`：验证命令、验证结果、失败修复记录、剩余未验证项

如果任务被明确判定为简单任务，可以压缩到单一任务文档中，但仍必须保留 Research / Decision / Risk / Verify 的最小记录；高风险任务不得压缩。
...
- 目标：基于 Requirement Contract 和 Evidence Map 形成可验证的初步方案
- 必需产物：`context.md`、`plan.draft.md`
- `context.md` 必须记录项目背景、现有代码入口、版本约束、运行环境、权限边界和已知限制
- `plan.draft.md` 必须记录初步实现策略、计划修改文件、计划新增代码、计划复用代码、风险假设和验证方向
- 核心规则：`plan.draft.md` 不是执行依据，只能作为技术事实验证输入
- 停止条件：缺少 `context.md`；缺少 `plan.draft.md`；初步设计没有列出关键技术假设；计划直接跳到实现；设计没有说明验证方式

## Research：技术事实验证门禁

- 目标：在完成初步设计后，验证关键技术假设，避免带着过期 API、不兼容依赖或错误运行环境进入实现
- 必需产物：`research.md`、`decision.md`、`risks.md`、更新后的 `plan.md`
- 核心规则：技术验证不是附录，而是设计门禁；只要 `research.md` 中出现高风险或不兼容结论，必须回写 `plan.md` 和 `tasks.md`
- 禁止行为：查完资料仍按原 `plan.draft.md` 实现；把 AI 内部知识当最终依据；带着已知不兼容设计继续实现
- 停止条件：关键技术假设未验证；不兼容风险未记录；替代方案未比较；设计文件未根据验证结果修正；未解决风险没有转化为显式任务或验收项

### 技术验证红线

出现以下想法或行为时，必须停止并回到 Research / Decision：

| 合理化借口 | 现实 |
|---|---|
| “这个改动很小，不需要 research” | 小改动也可能踩中版本、API、权限或运行环境限制；至少保留最小 Research 记录 |
| “先实现，后面再补 research” | 后补 research 不能阻止错误设计进入代码；实现前必须完成验证 |
| “我记得这个 API 可用” | 模型记忆只能辅助判断，不能作为最终依据 |
| “官方文档不用看，社区都这么写” | 官方文档和当前项目版本优先级高于社区实践 |
| “有风险但先按原方案写” | 高风险或不兼容结论必须回写设计和任务，不能继续执行原草案 |
| “PoC 太慢，直接做完整实现” | 关键假设高风险时，先做最小可验证 PoC 或把失败切换方案写入任务 |

### 需要验证的内容

1. 框架 / 平台版本是否支持该方案
2. 关键 API 是否仍然可用，是否已废弃
3. 第三方依赖是否仍维护
4. 是否存在更推荐的新方案
5. 是否存在安全策略、沙箱、权限、运行环境限制
6. 当前项目已有实现是否与设计冲突
7. 是否需要调整设计、替换依赖或降低实现范围

### 验证对象优先级

1. 当前项目代码、`package.json`、lockfile、`tsconfig` 和运行配置
2. 官方文档
3. 官方 release notes / changelog
4. GitHub issue / discussion
5. 主流社区实践
6. AI 内部知识只能作为辅助，不能作为最终依据

### 快速事实检查项

每次实现前必须强制检查：

1. 当前项目版本：`package.json`、lockfile、`tsconfig`、Electron / Node / Chromium、NestJS / TypeORM / Vite / Playwright 等相关版本
2. 官方能力：当前版本是否支持、API 是否废弃、是否有 breaking change
3. 替代方案：当前方案是否仍推荐、是否有更稳定方案、是否应抽象接口避免绑定单一实现
4. 验证方式：如何快速证明方案可行、是否先写 PoC、成功标准是什么、失败后切换什么方案
...
| 编号 | 技术假设 | 风险等级 | 是否必须验证 |
|---|---|---:|---|
| H1 | Electron 可继续使用 BrowserWindow 加载目标站点完成签名 | 高 | 是 |
| H2 | 当前 Playwright 可连接用户默认 Chrome 并监听请求 | 中 | 是 |
| H3 | 当前 sqlite 支持 JSON 字段局部更新 | 高 | 是 |
| H4 | 某 npm 依赖仍可用于当前 Node 版本 | 中 | 是 |

## 2. 验证结果

### H1：Electron BrowserWindow 方案

结论：部分可行，但存在稳定性风险。

依据：

- 当前 Electron 版本对安全上下文、跨域、webSecurity、session 隔离有更多限制
- 目标站点可能检测 WebView / Electron 环境
- 签名失败可能不是代码错误，而是页面环境、cookie、风控、脚本执行时机导致

建议：

- 不把 Electron 内嵌页面作为唯一签名通道
- 增加备用方案：连接真实 Chrome / CDP / 浏览器扩展 / 用户态浏览器会话
- 对签名失败增加重试、降级、健康检测
...
> 通过 Electron 加载 douyin.com 获取签名参数。

修正后：

> 签名能力抽象为 SignProvider，ElectronBrowserProvider 只是其中一个实现。后续可增加 ChromeCDPProvider、ExtensionProvider、RemoteBrowserProvider。
...
- 目标：把 Research 结论转成明确技术选择、范围调整和验收影响
- 必需产物：`decision.md`、`risks.md`
- `decision.md` 必须记录采用方案、放弃方案、取舍理由、依据来源和实现影响
- `risks.md` 必须记录剩余风险、触发条件、规避方案、降级方案和对应验收项
- 核心规则：高风险结论必须有设计调整；无法消除的风险必须转化为任务、验收项或明确不包含范围
- 停止条件：替代方案没有比较；高风险项没有决策；剩余风险没有规避方案；Research 结论没有回写到计划

## Plan：验证后的 Implementation Plan

- 目标：在编码前固定实现边界、风险与验证方式
- 必需产物：更新后的 `plan.md`，包含实现策略、计划修改文件、计划新增代码、计划复用代码、protected paths、forbidden actions、风险分析、Validation Plan
- 核心规则：只允许修改计划内文件；计划外文件如需修改必须先更新计划
- 停止条件：没有计划修改文件清单；没有计划复用说明；没有 forbidden actions；已知风险没有处理方式；没有 Validation Plan；没有 protected paths；计划范围超过 Requirement Contract；没有 Research / Decision 回写记录；计划仍依赖已知不兼容方案

## Spec：可执行规格

- 目标：把验证后的设计转成可执行、可验收的规格
- 必需产物：`spec.md`
- `spec.md` 必须包含接口/模块边界、数据结构、关键流程、错误处理、权限边界、验收标准和非目标
- 核心规则：规格必须来自 `plan.md`，不能绕过 Research / Decision 另起炉灶
- 停止条件：规格与 `plan.md` 冲突；验收标准不可测试；权限或运行环境边界缺失

## Tasks：实现任务拆解

- 目标：把 `spec.md` 拆成可顺序执行、可回写状态的任务
- 必需产物：`tasks.md`
- `tasks.md` 必须包含任务清单、依赖顺序、涉及文件、验收方式、风险任务标记
- 核心规则：任务必须承接 Research 中的风险处理项；高风险或不兼容修正必须体现为显式任务或验收项
- 停止条件：任务未覆盖 `spec.md`；任务跳过高风险项；任务依赖顺序不清；没有验收方式

## Implement：受控编码

- 目标：在既定计划和流程图契约下进行受控实现
- 前置条件：必须已有验证后的 `plan.md`、`spec.md`、`tasks.md`；不得使用 `plan.draft.md` 直接实现
- 核心规则：只按计划修改；每完成一个节点都必须输出对照块并回写流程图文件；发现计划外文件需要修改时先更新计划、规格和任务
- 停止条件：需要修改数据库结构；需要修改权限模型；需要修改跨服务接口契约；现有 Service 无法满足核心能力；同一问题连续修复 3 次仍失败；实现依据仍是 `plan.draft.md`；发现 Research 结论与实现路径冲突
...
- 目标：用命令级验证结果证明代码有效
- 必需产物：`verify.md`，包含验证项、验证命令、通过 / 未通过、失败说明、失败修复记录
...
6. Plan Draft：生成 `context.md` 和 `plan.draft.md`；草案只用于验证，不得实现
7. Research：生成 `research.md`，验证关键技术假设、版本、API、依赖、运行环境和项目冲突
8. Decision：生成 `decision.md` 和 `risks.md`，完成替代方案比较、技术取舍和风险处理
9. Plan：把 Research / Decision 结论回写成正式 `plan.md`
10. Spec：生成 `spec.md`，固定可执行规格和验收标准
11. Tasks：生成 `tasks.md`，拆解实现任务、依赖顺序和风险处理任务
12. Implement：只按 `plan.md`、`spec.md`、`tasks.md` 受控编码
13. Review：复核 diff 是否符合 Requirement Contract、Evidence Map、Research、Decision、Plan 和权限边界
14. Validate：运行自动验证并记录到 `verify.md`；失败项必须有修复记录
15. Deliver：输出完成内容、未包含内容、验证说明、剩余风险和后续建议
16. 失败时根据问题性质回流：合同问题回 Confirm，证据问题回 Locate，技术事实问题回 Research / Decision，计划问题回 Plan，规格或任务问题回 Spec / Tasks，验证失败回 Validate；修正后重新 Review / Validate
```
...
主线顺序是：先确认、再查证、再做设计草案、再技术事实验证、再决策、再形成正式计划和规格任务、再实现、再复核、再验证、再交付。
